var tmpl =  "<li>"+
              "<input id='ID' onclick='handleClick(this)' type='checkbox'/><span>TEXT</span><br/><br/><center><button onclick='removeThis(this)'>Delete</button></center>"+
            "</li>";
...
$(document).ready(function(event){
  ...
  if(JSON.parse(localStorage.getItem("data"))==null){
    ...
    for(var t=0;t<data.length;t++){
      ...
      if(data[t].complete==true){
        $(".list").append(object);
        $("#"+data[t].id).parent().children("span").addClass('complete');
  ...
  $("#clear").on('click',function(){
    ...
    $(".list > li").each(function(){
      if($(this).children("span").hasClass("complete")){
        $(this).remove();
      }else{
...
function removeThis(b){
  var n = $(b).parent().parent().children("input").attr("id");
  var ndata = new Array();
  for(var t=0;t<data.length;t++){
    if(data[t].id!=n)
      ndata.push(data[t]);
